...
function SubjectTeacherDashboard({ teacherId }: { teacherId: string }) {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        if (!cancelled) setAssignments(Array.isArray(json) ? (json as Assignment[]) : []);
      } catch {
  ...
  const deadlineBySubjectId = useMemo(() => {
    ...
    for (const a of assignments) {
      ...
      const d = s && typeof s === "object" ? String((s as any).deadline ?? "").trim() : "";
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        <div className="space-y-1">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-3xl font-bold tracking-tight">Dashboard Guru Subjek</h1>
          </div>
          <p className="text-muted-foreground">Ringkasan tugasan pemarkahan & pautan pantas</p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <MetricCard title="Subjek Diajar" value={subjectCount} icon={BookOpen} />
          <MetricCard title="Jumlah Kelas" value={classCount} icon={Users} tone="secondary" />
          <MetricCard title="Tugasan" value={assignments.length} icon={ClipboardList} tone="accent" />
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <Card className="shadow-lg border border-border/50">
            ...
            <CardContent className="space-y-3">
              <QuickLink href="/teacher/my-subject" icon={ClipboardList} title="Masuk Markah" desc="Pemarkahan subjek & hantar untuk semakan" />
              <QuickLink href="/teacher/omr" icon={Camera} title="Imbasan OMR" desc="Ambil gambar & proses OMR" />
              <QuickLink href="/teacher/report" icon={FileText} title="Laporan" desc="Ringkasan keputusan peperiksaan" />
            </CardContent>
          </Card>
        </div>
      </div>
...
function ClassTeacherDashboard({ teacherId }: { teacherId: string }) {
  ...
  const className = data?.class?.name ?? "â€”";
  const grade = data?.class?.grade ?? "â€”";
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        <div className="space-y-1">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-3xl font-bold tracking-tight">Dashboard Guru Kelas</h1>
          </div>
          <p className="text-muted-foreground">Ringkasan kelas & pengurusan pelajar</p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <MetricCard title="Kelas" value={className} icon={Users} />
          <MetricCard title="Tingkatan" value={grade} icon={GraduationCap} tone="secondary" />
          <MetricCard title="Bil. Pelajar" value={studentCount} icon={Users} tone="accent" />
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <TableBody>
                  {(data?.students ?? []).slice(0, 6).map((s) => (
                    <TableRow key={s.id}>
                      ...
                      <TableCell className="text-muted-foreground">{s.identifier}</TableCell>
                    </TableRow>
                  ))}
          ...
          <Card className="shadow-lg border border-border/50">
            ...
            <CardContent className="space-y-3">
              <QuickLink href="/teacher/my-class" icon={Users} title="Pelajar Kelas Saya" desc="Tambah/keluar pelajar & simpan comment report card" />
              <QuickLink href="/teacher/reports" icon={FileText} title="Laporan" desc="Semak laporan dan ringkasan" />
              <Button className="w-full justify-between" variant="outline" onClick={() => router.push("/teacher/my-class")}>
