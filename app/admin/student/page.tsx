...
export default function StudentsPage() {
  ...
  const filteredStudents = useMemo(() => {
    const list = students
      .filter((student) => {
        ...
        const studentForm = estimateFormFromIC(student.identifier) || "Tidak pasti";
        const matchesForm =
      ...
      .sort((a, b) => {
        ...
        if (sortBy === "name") {
          ...
          compareA = (estimateFormFromIC(a.identifier) || "").toLowerCase();
          compareB = (estimateFormFromIC(b.identifier) || "").toLowerCase();
        }

        if (sortOrder === "asc") return compareA.localeCompare(compareB);
  ...
  const stats = useMemo(() => {
    ...
    const forms = students.map((s) => estimateFormFromIC(s.identifier)).filter(Boolean);
    const formCounts = {
              ...
              <div className="w-full sm:w-[220px]">
                <Select value={selectedForm} onValueChange={setSelectedForm}>
                  ...
                  <SelectContent className="rounded-lg border-border">
                    ...
                    <SelectItem value="Tidak pasti">Tidak pasti</SelectItem>
                  </SelectContent>
                </Select>
              </div>
                  ...
                  <TableBody>
                    {loading ? (
                          ...
                          <div className="flex flex-col items-center justify-center gap-4">
                            ...
                            <div className="text-center">
                              ...
                              <p className="text-sm text-muted-foreground mt-1 max-w-md">
                                Tiada data sepadan dengan carian/penapisan anda
                              </p>
                            </div>
                          </div>
                        </TableCell>
                      ...
                      filteredStudents.map((student, index) => {
                        const form = estimateFormFromIC(student.identifier) || "Tidak pasti";
          ...
          <DialogContent className="rounded-lg border-border sm:max-w-[500px]">
            ...
            <form onSubmit={handleSaveEdit} className="space-y-4 pt-2">
              <div className="space-y-3">
                ...
                <div className="space-y-2">
                  <Label htmlFor="edit-identifier" className="font-medium">
                    No. Kad Pengenalan
                  </Label>
                  <Input
                    id="edit-identifier"
                ...
                <div className="space-y-2">
                  <Label htmlFor="edit-class" className="font-medium">
                    Kelas
                  </Label>
                  <Select
                    value={editForm.className}
                    onValueChange={(value) =>
                      setEditForm({ ...editForm, className: value })
                    }
                  >
        ...
        <Dialog open={!!deletingStudent} onOpenChange={(open) => !open && setDeletingStudent(null)}>
          <DialogContent className="rounded-lg border-border">
            <DialogHeader>
              ...
              <DialogDescription className="text-muted-foreground">
                Anda pasti ingin memadam pelajar{" "}
                <b className="text-foreground">{deletingStudent?.name}</b>?
              </DialogDescription>
            </DialogHeader>
