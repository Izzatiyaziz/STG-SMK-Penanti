...
import { formatMalaysiaTime, getMalaysiaDateInputValue } from "@/lib/date-utils";
...
export default function ExamsPage() {
    ...
    const academicYears = useMemo(() => {
        return Array.from(new Set([CURRENT_YEAR, ...exams.map((exam) => exam.academic_year).filter(Boolean)])).sort(
    ...
    const filteredExams = useMemo(() => {
        ...
        return exams
            .filter((exam) => {
                ...
                const matchesYear = yearFilter === "all" || exam.academic_year === yearFilter;

                return matchesSearch && matchesYear;
            })
            .sort((a, b) =>
                sortOrder === "asc"
                    ? a.academic_year.localeCompare(b.academic_year) || a.name.localeCompare(b.name)
    ...
    async function handleAddExam(e: React.FormEvent<HTMLFormElement>) {
        ...
        const academic_year = normalizeSpaces(String(formData.get("academic_year") ?? ""));
    ...
    async function handleEditExam(e: React.FormEvent<HTMLFormElement>) {
        ...
        try {
            ...
            if (!res.ok) {
                toast.error(data.message || "Gagal mengemas kini peperiksaan", { id: toastId });
                return;
            }
    ...
    async function handleSaveSettings() {
        ...
        try {
            ...
            if (!res.ok) {
                toast.error(data.message || "Gagal menyimpan tetapan peperiksaan", { id: toastId });
                return;
            }
    ...
    const hasFilters = Boolean(searchQuery.trim()) || yearFilter !== CURRENT_YEAR || sortOrder !== "desc";
    const fromItem = filteredExams.length === 0 ? 0 : (currentPage - 1) * PAGE_SIZE + 1;
    ...
    return (
        <div className="min-h-screen bg-background p-4 md:p-6">
                        ...
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                ...
                                <p className="mt-1 font-medium text-muted-foreground">
                                    Mengurus senarai peperiksaan, tahun akademik dan tetapan markah mengikut subjek
                                </p>
                            </div>
                        </div>
                        ...
                        <div className="flex flex-col justify-between gap-4 md:flex-row md:items-center">
                            <div>
                                ...
                                <p className="mt-1 text-sm text-muted-foreground">
                                    Urus dan pantau semua peperiksaan dalam sistem
                                </p>
                            </div>
                            ...
                            <div className="flex items-center gap-3">
                                <Badge variant="outline" className="border-primary/30 bg-primary/5 font-medium text-primary">
                                    ...
                                    {filteredExams.length} peperiksaan
                                </Badge>
                            </div>
                        </div>
                    </CardHeader>
                                ...
                                <Table>
                                    <TableHeader className="bg-muted/30">
                                        <TableRow className="border-b border-border hover:bg-transparent">
                                            ...
                                            <TableHead className="py-4 font-semibold text-foreground">
                                             <Button variant="ghost" size="sm" onClick={() => setSortOrder((current) => (current === "asc" ? "desc" : "asc"))}
                                                ...
                                                <TableCell colSpan={5} className="py-16">
                                                    <div className="flex flex-col items-center justify-center gap-4">
                                                        ...
                                                        <div className="text-center">
                                                            <p className="font-semibold text-foreground">Memuatkan data peperiksaan...</p>
                                            ...
                                            <TableRow>
                                                <TableCell colSpan={5} className="py-16">
                                                    <div className="flex flex-col items-center justify-center gap-4">
                                                        ...
                                                        <div className="text-center">
                                                            <p className="font-semibold text-foreground">Tiada peperiksaan dijumpai</p>
                                            ...
                                            paginatedExams.map((exam, index) => {
                                                ...
                                                return (
                                                    ...
                                                    >
                                                        ...
                                                        >
                                                            <div className="flex items-center gap-3">
                                                                <div className="flex h-10 w-10 items-center justify-center rounded-full border border-primary/20 bg-gradient-to-br from-primary/20 to-primary/10 shadow-xs">
                ...
                <DialogContent className="sm:max-w-[500px]">
                    ...
                    <form onSubmit={handleEditExam} className="space-y-5 py-4">
                        <div className="space-y-4">
                            <div className="space-y-2">
                                <Label>Nama Peperiksaan</Label>
                                <Input name="exam_name" defaultValue={editing?.name} required className="h-11" />
                            </div>
                            <div className="space-y-2">
                                <Label>Tahun Akademik</Label>
                                <Input name="academic_year" defaultValue={editing?.academic_year} required className="h-11" />
                            </div>
                        </div>
                        <DialogFooter>
            ...
            <Dialog open={Boolean(deleting)} onOpenChange={(open) => !open && setDeleting(null)}>
                <DialogContent>
                    <DialogHeader>
                        <DialogTitle className="font-bold text-destructive">Padam Peperiksaan?</DialogTitle>
                        <DialogDescription>
                            Padam rekod <strong>{deleting?.name}</strong>? Data berkaitan peperiksaan ini juga akan dipadam.
                        </DialogDescription>
                    </DialogHeader>
                    <DialogFooter>
            ...
            <Dialog open={Boolean(settingExam)} onOpenChange={(open) => !open && setSettingExam(null)}>
                <DialogContent className="sm:max-w-[640px]">
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            Tetapkan tarikh akhir hantar markah serta had markah objektif dan subjektif.
                        </DialogDescription>
                    </DialogHeader>
                    ...
                    <div className="space-y-5 py-4">
                        ...
                        <div className="rounded-lg border border-amber-100 bg-amber-50 p-3">
                            <p className="text-[11px] leading-relaxed text-amber-700">
                                <strong>Nota:</strong> Tetapan ini digunakan oleh guru ketika merekod markah peperiksaan untuk subjek yang dipilih.
                            </p>
                        </div>
                    </div>
                    ...
                    <DialogFooter>
                        ...
                        <Button onClick={handleSaveSettings} disabled={!selectedSubjectId} className="px-8">
                            Simpan
                        </Button>
                    </DialogFooter>
                </DialogContent>
