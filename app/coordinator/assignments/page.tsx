...
export default function SubjectCoordinatorAssignmentsPage() {
  ...
  const [pendingTeacherByClass, setPendingTeacherByClass] = useState<Record<string, string>>({});
  ...
  useEffect(() => {
    ...
    async function loadSubjects() {
      try {
        const res = await fetch(`/api/coordinator/subjects?teacher_id=${teacherId}`);
        const json = await res.json();
        const list: Subject[] = (json?.data ?? []).map((s: any) => ({
          id: String(s.id ?? ""),
          name: String(s.name ?? ""),
          ...
          <CardContent className="flex flex-col gap-4 p-5 sm:flex-row sm:items-center sm:justify-between sm:p-6">
            <div className="min-w-0">
              ...
              <h3 className="mt-2 truncate text-xl font-semibold">
                {data?.coordinator?.name ?? "â€”"}
              </h3>
              <p className="text-sm text-muted-foreground mt-1">
                Subjek: {data?.subject?.name ?? "â€”"}
              </p>
            </div>
                      ...
                      <TableCell>
                        ...
                        {assignedTeacherId && (
                          <div className="text-xs text-muted-foreground mt-1">
                            Current: {teacherNameById.get(assignedTeacherId) ?? "â€”"}
                          </div>
                        )}
                      </TableCell>
