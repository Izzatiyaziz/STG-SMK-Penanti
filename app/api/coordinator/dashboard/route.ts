...
export async function GET(req: Request) {
    try {
        ...
        const classById = new Map<string, { id: string; name: string; grade: number }>();
        for (const c of Array.isArray(classes) ? classes : []) {
            ...
            if (id) classById.set(id, { id, name, grade: Number.isFinite(grade) ? grade : 0 });
        }
        ...
        const studentsByClassId = new Map<string, Array<{ id: string; name: string }>>();
        ...
        const classSummaries = (assignments ?? [])
            .map((a: any) => {
                ...
                for (const s of studentsInClass) {
                    ...
                    if (rr?.hasSubjective) {
                        resultsCount += 1;
                        totalSum += rr.total;
                        if (rr.grade && gradeCounts[rr.grade] !== undefined) gradeCounts[rr.grade] += 1;
                        if (rr.status && statusCounts[rr.status] !== undefined) statusCounts[rr.status] += 1;
                    }
                }
            ...
            .sort((x, y) => (x.grade - y.grade) || x.class_name.localeCompare(y.class_name));

        const totalClasses = classSummaries.length;
        const totalTeachers = new Set(classSummaries.map((c) => c.teacher_id).filter(Boolean)).size;
        ...
        for (const c of classSummaries) {
            if (c.total_students > 0 && c.submitted_count === 0) {
                alerts.push(`${c.class_name} â€” Tiada markah dihantar`);
            } else if (c.total_students > 0 && c.submitted_count < c.total_students) {
                alerts.push(`${c.class_name} â€” Belum lengkap (${c.submitted_count}/${c.total_students})`);
            }
            if (c.average_total > 0 && c.average_total < 50) {
                alerts.push(`${c.class_name} â€” Purata bawah 50%`);
            }
        }
        ...
        return NextResponse.json({
            coordinator: coordTeacher
                ...
                : null,
            subject: subject ? { id: subject.subject_id, name: subject.subject_name } : null,
            exam: exam
